...
class TimeSeries():
    ...
    def time_normalize(self, event_name1, event_name2, n_points=100):
        """
        Time-normalize the TimeSeries
        ...
        n_cycles = np.min([
                np.sum(np.array(self.events)[:, 1] == event_name1),
                np.sum(np.array(self.events)[:, 1] == event_name2)])
        ...
        for key in self.data.keys():
            new_shape = list(self.data[key].shape)
        ...
        for i_cycle in range(n_cycles):
            ...
            for key in subts.data.keys():
                dest_ts.data[key][n_points * i_cycle:
                        n_points * (i_cycle+1)] = subts.data[key]


        # Assign the dest_ts data to self
        self.time = dest_ts.time
        self.data = dest_ts.data
        self.events = dest_ts.events

    def get_reshaped_time_normalized_data(self, n_points=100):
        """
        Get reshaped time-normalized data.
        ...
        """
        if np.mod(len(self.time), n_points) != 0:
            raise(ValueError(
                    'It seems that this TimeSeries is not time-normalized.'))

        data = dict()
        for key in self.data.keys():
            current_shape = self.data[key].shape
            new_shape = [-1, n_points]
            for i in range(1, len(current_shape)):
                new_shape.append(self.data[key].shape[i])
            data[key] = self.data[key].reshape(new_shape, order='C')
        return data
