...
def create_reference_frame(global_points, method='ocx1'):
    ...
    if method == 'ocx1':
        ...
        x = np.cross(y, z)
#
#    elseif strcmpi(method, 'o1z2')
#
#        % Origin
#        origin = globalPoints(:,1,:);
#
#        % Z axis
#        z = globalPoints(:,2,:) - origin;
#        z = ktkGeometry.normalize(z);
#
#        % Y plane
#        xtemp = globalPoints(:,3,:) - origin;
#        xtemp = ktkGeometry.normalize(xtemp);
#
#        % Y axis
#        y = ktkGeometry.cross(z, xtemp);
#        y = ktkGeometry.normalize(y);
#
#        % X axis
#        x = ktkGeometry.cross(y, z);
#
#    elseif strcmpi(method, 'isb-humerus')
#
#        % Origin
#        origin = globalPoints(:,1,:);
#
#        % Y axis
#        y = origin - globalPoints(:,2,:);
#        y = ktkGeometry.normalize(y);
#
#        % YF axis
#        yf = globalPoints(:,3,:) - globalPoints(:,2,:);
#        yf = ktkGeometry.normalize(yf);
#
#        % Z axis
#        z = ktkGeometry.cross(yf, y);
#        z = ktkGeometry.normalize(z);
#
#        % X axis
#        x = ktkGeometry.cross(y, z);
#        x = ktkGeometry.normalize(x);
#
#    elseif strcmpi(method, 'isb-forearm')
#
#        % Origin
#        origin = globalPoints(:,1,:);
#
#        % Y axis
#        y = globalPoints(:,2,:) - origin;
#        y = ktkGeometry.normalize(y);
#
#        % Z axis
#        z = ktkGeometry.cross((globalPoints(:,3,:) - origin(:,1,:)), y);
#        z = ktkGeometry.normalize(z);
#
#        % X axis
#        x = ktkGeometry.cross(y, z);
...
#
#    else
#        error('KTK: This method is not supported.');
#    end
...
def get_local_coordinates(global_coordinates, reference_frame):
    ...
    for i_sample in range(ref_t.shape[0]):
        inv_ref_t[i_sample] = inv_ref_rot[i_sample] @ -ref_t[i_sample]

    inv_ref_T = np.zeros((ref_t.shape[0], 4, 4))  # init
